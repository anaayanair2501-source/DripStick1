...
interface ChocolateDripDividerProps {
  ...
  inverted?: boolean;
  showDrops?: boolean;
  accentType?: 'chocolate' | 'strawberry' | 'caramel' | 'pistachio' | 'blue' | 'purple' | 'orange' | 'rose' | 'yellow' | 'teal' | 'fuchsia' | 'mint' | string;
...
export const ChocolateDripDivider: React.FC<ChocolateDripDividerProps> = ({
  ...
  return (
    <div className={`relative w-full overflow-hidden leading-none z-10 border-t border-[#4A2C2A]/15 ${inverted ? 'rotate-180' : ''} ${className}`}>
      ...
      {secondaryFillColor && (
        <svg
          ...
          className="absolute inset-0 block w-full h-12 sm:h-16 lg:h-20 opacity-40"
        >
          <path
